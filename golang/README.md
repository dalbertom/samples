...
#### Range and Close
tour/range-and-close.go
* A sender can `close` a channel to indicate that no more values will be sent.
* Receivers can test whether a channel has been closed by assigning a second parameter to receive the expression
```
v, ok := <-ch
```
...
* A `select` blocks until one of its cases can run, then it executes that case. It chooses one random if multiple are ready.

#### Default Selection
tour/default-selection.go
...
#### Exercise: Equivalent BinaryTrees
tour/exercise-equivalent-binary-trees.go
...
* For example, here are two binary trees storing the sequence 1, 1, 2, 3, 5, 8, 13
  * Tree(3, Tree(1, Tree(1), Tree(2)), Tree(8, Tree(5), Tree(13)))
  * Tree(8, Tree(3, Tree(1, Tree(1), Tree(2)), Tree(5)), Tree(13))
